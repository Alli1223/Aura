...
- [ ] **watcher-crud-wiring** — Call `refreshLibraryWatcher()` from library create/update/delete routes so watchers track new/removed libraries immediately (today only the periodic scheduler picks them up)
- [ ] **media-browse-api** — Read API the web app browses: `GET /api/libraries/:id/items` (paginated/sorted/filtered list with poster + watch-state overlays), `GET /api/items/:id` (movie detail, or show→seasons→episodes with files/streams/genres), recently-added & continue-watching feeds; all access-checked with the 404 cloak. Prerequisite for library-browse, media-detail, home-screen, search
